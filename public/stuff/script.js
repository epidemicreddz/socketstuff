...
function parseNotes(input) {
    ...
    lines.forEach(line => {
        ...
        if (match) {
            const time = parseInt(match[1], 10) - 10000;
            const lane = parseInt(match[2], 10) - 1;
            notes.push({ time, lane});
        }
    });

    return JSON.stringify(notes, null, 4); // Pretty print JSON
}
...
let id = [0, 0, 0, 0];
let hit = [0, 0, 0, 0];
let missCount = [0, 0, 0, 0];

function CreateOffset(note, dist) {
	const lineee = document.createElement("div");
    lineee.classList.add("linee");
    lineee.style.left = note.getBoundingClientRect().left + "px";
    lineee.style.top = note.getBoundingClientRect().top + "px"; 
...
async function CalculateColor(laneIndex) {
    lanes[laneIndex].querySelectorAll(".note").forEach(note => { // Now only checking notes in its lane
        const noteColor = parseInt(note.dataset.id);
        if (hit[laneIndex] === noteColor) {
            const dist = GetDistance(note);
            if (Math.abs(dist) < 150) {
                CreateOffset(note, dist);
                note.remove();
			} else if (Math.abs(dist) >= 150 || isNaN(dist)){
                hit[laneIndex] -= 1;
            }
        }
    });
...
function realignLane(laneIndex) {
    console.log(`Realigning lane ${laneIndex} due to offset.`);
    hit[laneIndex] = id[laneIndex]; // Reset hit count to match spawned notes
}
...
setTimeout(() => {
    if (document.contains(note)) {
        ...
        if (hit[laneIndex] < id[laneIndex]) { 
            hit[laneIndex] += 1;  // Ensures we don't overshoot
        }
    }
}, 650);
...
function NoteStuff(laneIndex) {
    ...
    if (hit[laneIndex] < id[laneIndex]) {
        CalculateColor(laneIndex); // Check before incrementing
        hit[laneIndex] += 1;
    }
}
...
const notes = JSON.parse(parseNotes(inputText));
notes.forEach(note => {
...
// Call realignment every 5 seconds
setInterval(realignLanes, 1000);
